...
<style>
    ...
    .main-header {
        ...
        color: #0F2942; letter-spacing: -1px; margin-bottom: 0.1rem;
    }
    ...
    .pred-card {
        ...
        color: white; padding: 1.2rem 1rem; border-radius: 14px;
...
def lstm_one_step(lstm_model, temp, humidity, wind_speed, pressure,
    ...
    try:
        ...
        scaler = MinMaxScaler()
        scaler.fit(window)
        X        = scaler.transform(window).reshape(1, LOOKBACK, 4)
...
def run_predictions(temp, humidity, wind_speed, pressure, temp_yesterday, temp_2days_ago):
    ...
    if models['lstm']:
        v = lstm_one_step(models['lstm'], temp, humidity, wind_speed,
                          pressure, temp_yesterday, temp_2days_ago)
        if v: results['LSTM'] = v
    if models['arima']:
        v = arima_one_step(models['arima'], temp_yesterday)
        if v: results['ARIMA'] = v
    if models['sarima']:
        v = arima_one_step(models['sarima'], temp_yesterday)
        if v: results['SARIMA'] = v

    RMSE = {'XGBoost':2.10,'LightGBM':2.20,'LSTM':2.50,'ARIMA':3.20,'SARIMA':3.00}
    w    = {k: 1/RMSE[k] for k in results if k in RMSE}
    tw   = sum(w.values())
    if tw > 0:
...
with left:
    if predict_btn:
        ...
        if chart_keys:
            ...
            bars   = ax.bar(chart_keys, chart_vals, color=colors,
            ...
            ax.set_title('All Model Predictions', fontsize=11, fontweight='bold',
                         color='#0F2942', pad=10)
            for bar, val in zip(bars, chart_vals):
                ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.15,
                        f'{val:.1f}°C', ha='center', va='bottom',
                        fontsize=9, fontweight='600', color='#0F2942')
        ...
        for name in MODEL_ORDER:
            ...
            rows.append({'Model': name, 'Type': TYPE_INFO[name],
                         'Prediction': f"{pred:.1f}°C" if pred else 'N/A',
                         'Condition':  condition(pred) if pred else '—',
        ...
        <div style="background:#F0F7FF;border-radius:14px;padding:2rem;
            ...
            <div style="font-size:1.2rem;font-weight:600;color:#1E3A5F;margin:0.5rem 0">
                Ready to Forecast
            </div>
        ...
        st.markdown('<div class="section-title" style="margin-top:1.5rem">Model Status</div>',
                    unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
...
with right:
    ...
    Mean daily temp in °C. Delhi: ~6°C (Jan) to ~40°C (May–Jun).</div>
    <div class="info-box"><b>💧 Humidity</b><br>
    Relative humidity %. Peaks in monsoon (Jul–Sep).</div>
    <div class="info-box"><b>💨 Wind Speed</b><br>
    Mean daily wind in km/h. Strong winds often precede storms.</div>
    <div class="info-box"><b>🌀 Pressure</b><br>
    Atmospheric pressure in hPa. Low = rain likely. Normal = 1005–1020.</div>
    <div class="info-box"><b>📅 Recent History</b><br>
    Yesterday + 2-days-ago temp feed lag features for XGB/LGB
    and context window for ARIMA/SARIMA.</div>
    """, unsafe_allow_html=True)

    st.markdown('<div class="section-title" style="margin-top:1.2rem">Model Reference</div>',
                unsafe_allow_html=True)
    st.dataframe(pd.DataFrame({
        'Model'  : ['XGBoost','LightGBM','LSTM','ARIMA','SARIMA','Ensemble'],
        'RMSE'   : ['~2.1°C','~2.2°C','~2.5°C','~3.2°C','~3.0°C','~2.0°C'],
        'R²'     : ['~0.95','~0.94','~0.93','~0.88','~0.90','~0.96'],
    }).set_index('Model'), use_container_width=True)

    st.markdown('<div class="section-title" style="margin-top:1.2rem">Ensemble Strategy</div>',
                unsafe_allow_html=True)
    st.markdown("""
    <div class="info-box">
    Uses <b>inverse-RMSE weighting</b> — lower-error models contribute more.
    If optional models (LSTM/ARIMA/SARIMA) are missing, weights automatically
    renormalise across the available models.
    </div>""", unsafe_allow_html=True)

    if predict_btn and 'results' in dir():
        st.markdown('<div class="section-title" style="margin-top:1.2rem">Input Summary</div>',
                    unsafe_allow_html=True)
        st.dataframe(pd.DataFrame({
            'Parameter': ['Temperature','Humidity','Wind Speed','Pressure',
                          'Temp Yesterday','Temp 2 Days Ago'],
